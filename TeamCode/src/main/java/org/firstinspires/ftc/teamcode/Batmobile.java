...
@TeleOp
public class Batmobile extends OpMode {

    DcMotor fe, fd, td, te;
    DcMotor intake;
    DcMotor launcher;

    // ===== MOTOR DA TORRETA / CÂMERA =====
    DcMotor cameraMotor;
    ...
    // ===== CONTROLE DA TORRETA (AJUSTADO) =====
    final double kP = 0.06;        // ganho maior = mais força
    final double DEAD_DEG = 1.0;   // zona morta
    final double MIN_POWER = 0.25; // torque mínimo (ESSENCIAL)
    final double MAX_POWER = 0.6;  // limite de segurança
    ...
    public void loop() {
        ...
        // ================= ALINHAMENTO AUTOMÁTICO DA TORRETA =================
        ...
        if (viuTag) {
            double giroCamera = txDegrees * kP;

            if (Math.abs(txDegrees) < DEAD_DEG) {
                giroCamera = 0;
            } else {
                if (giroCamera > 0)
                    giroCamera = Math.max(giroCamera, MIN_POWER);
                else
                    giroCamera = Math.min(giroCamera, -MIN_POWER);
            }
